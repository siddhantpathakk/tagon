...
class TGAN(torch.nn.Module):
    def __init__(self, ngh_finder, n_feat, e_feat,
        ...
        if node_dim is None:
            ...
            self.n_feat_th = torch.nn.Parameter(torch.from_numpy(np.random.random((n_feat.shape[0], node_dim)).astype(np.float32)))
        print("node feat shape: ", self.n_feat_th.shape)
        print("node feat 1st row: ", self.n_feat_th[0, :])
        #self.e_feat_th = torch.nn.Parameter(torch.from_numpy(e_feat.astype(np.float32)))
        self.e_feat_th = torch.nn.Parameter(torch.from_numpy(np.zeros((e_feat.shape[0], node_dim), dtype=np.float32)))
        print("edge feat shape: ", self.e_feat_th.shape)
        print("edge feat 1st row: ", self.e_feat_th[0, :])
        ...
        self.model_dim = self.n_feat_dim + self.e_feat_dim + time_dim
        ...
        if agg_method == 'attn':
            ...
            self.attn_model_list = torch.nn.ModuleList([MeanPool(self.n_feat_dim,
