...
from components.trainer.trainer import Trainer
from components.trainer.trainer_utils import setup_model, setup_optimizer
from components.utils.parse import parse_training_args
from components.utils.utils import EarlyStopMonitor, make_pred_df, set_seed
...
print('Please note that the test mode is set to True. This means that the model will not be trained, but only tested on the test set/user.') if test_mode else print(
    'Testing mode is set to False. The model will be trained only.')
...
os.mkdir(SAVE_MODEL_DIR(args, cwd)) if not os.path.isdir(
    SAVE_MODEL_DIR(args, cwd)) else None
...
trainer.save_model(SAVE_MODEL_DIR(
    args)) if not test_mode and not infer_mode else None
