...
def make_ctbg(user_hist, dataset, trim=10):
    ...
    mid_y = (max(item_y_positions) + min(item_y_positions)) / \
        2 if item_y_positions else 0
    ...
    node_colors = ['#A4D4B4' if node in user_hist['u'].unique()
                   else '#E4D9FF' for node in B.nodes()]
    ...
    hover_texts = [
        ...
        f"Item ID: {node}<br>Item Name: {get_item_name(item_name_df, node)}<br>Timestamp: {user_hist[user_hist['i'] == node]['ts'].iloc[0]}"
        for node in B.nodes()
    ]
    ...
    fig.add_trace(go.Scatter(x=node_x, y=node_y, mode='markers+text', text=node_texts, marker=dict(size=30, color=node_colors),
                             hoverinfo='text', hovertext=hover_texts, textposition='top center',
