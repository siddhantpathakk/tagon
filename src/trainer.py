...
from utils.draw.draw import Draw_Bipartite_Graph
import pickle
from utils.draw.heatmap import HeatMap
    ...
    def train(self,train_part,test_part,eval_new_user=True):
        ...
        for epoch_ in range(self.arg.epoch_num):
            ...
            if (epoch_ +1) % 1 == 0:
                ...
                K = 10
                print(f'Epoch [{epoch_ + 1}/{self.arg.epoch_num}]:\tPrecision@{K}: {precision[K_map[K]]:.4f}\tRecall@{K}:  {recall[K_map[K]]:.4f}\tMAP@{K}: {MAP[K_map[K]]:.4f}\tNDCG@{K}: {ndcg[K_map[K]]:.4f}')