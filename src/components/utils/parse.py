...
def parse_training_args(dataset=None):
    ### Argument and global variables
    parser = argparse.ArgumentParser('Interface for TAGON experiments on link predictions')
    parser.add_argument('-d', '--data', type=str, help='data sources to use', default=dataset if dataset else 'ml-100k')
    
    parser.add_argument('--bs', type=int, default=150, help='batch_size')
    ...
    parser.add_argument('--infer_mode', action='store_true', help='inference mode for demo', default=None)
...
def parse_app_args(dataset):
    parser = argparse.ArgumentParser(
        'Interface for TAGON experiments on link predictions', fromfile_prefix_chars='@')
    parser.add_argument('-d', '--data', type=str, help='data sources to use, try wikipedia or reddit', default=dataset)
    parser.add_argument('--bs', type=int, default=150, help='batch_size')
    parser.add_argument('--prefix', type=str, default='', help='prefix to name the checkpoints')
    parser.add_argument('--n_degree', type=int, default=20, help='number of neighbors to sample')
    parser.add_argument('--n_head', type=int, default=2, help='number of heads used in attention layer')
