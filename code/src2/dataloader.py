...
class DataCollector:
    ...
    def prepare(self, verbose=True, logger=None):
        ...
        if verbose:
            ...
            logger.info(f'uid2locid_time: {len(uid2locid_time)} ')
            logger.info(f'locid2detail: {len(locid2detail)} ')
            logger.info(f'node_num: {node_num} ')
            logger.info(f'relation_num: {relation_num} ')
            logger.info(f'user_num: {len(uid_list_)} ')
            logger.info(f'train_num: {user_np.shape[0]} ')
            logger.info(f'test_num: {seq_test.shape[0]} ')
            logger.info(f'train_set: {seq_train.shape} ')
            logger.info(f'test_set: {seq_test.shape} ')
            logger.info(f'uid_list_: {len(uid_list_)} ')
            logger.info(f'u2v: {len(u2v)} ')
            logger.info(f'u2vc: {len(u2vc)} ')
            logger.info(f'v2u: {len(v2u)} ')
            logger.info(f'v2vc: {len(v2vc)} ')
