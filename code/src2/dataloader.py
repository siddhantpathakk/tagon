...
class DataCollector:
    ...
    def prepare(self, verbose=True, logger=None):
        uid2locid_time,locid2detail,uid2attr = self.main_data2dict()
        uid2locid_time,locid2detail,node_num,relation_num,uid_list_,user_np,seq_train,seq_test,test_set,u2v,u2vc,v2u,v2vc = self.main_datadict2traindata(uid2locid_time,locid2detail,uid2attr)
        
        if verbose:
            logger.info(f'[STATUS] Data preprocessing completed')
            logger.info(f'[INFO] uid2locid_time: {len(uid2locid_time)} ')
            logger.info(f'[INFO] locid2detail: {len(locid2detail)} ')
            logger.info(f'[INFO] node_num: {node_num} ')
            logger.info(f'[INFO] relation_num: {relation_num} ')
            logger.info(f'[INFO] user_num: {len(uid_list_)} ')
            logger.info(f'[INFO] train_num: {user_np.shape[0]} ')
            logger.info(f'[INFO] test_num: {seq_test.shape[0]} ')
            logger.info(f'[INFO] train_set: {seq_train.shape} ')
            logger.info(f'[INFO] test_set: {seq_test.shape} ')
            logger.info(f'[INFO] uid_list_: {len(uid_list_)} ')
            logger.info(f'[INFO] u2v: {len(u2v)} ')
            logger.info(f'[INFO] u2vc: {len(u2vc)} ')
            logger.info(f'[INFO] v2u: {len(v2u)} ')
            logger.info(f'[INFO] v2vc: {len(v2vc)} ')
        ...
        edges = [u2v, u2vc, v2u, v2vc]
        return train_part, test_part, info, edges