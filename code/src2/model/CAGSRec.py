...
from model.model_utils import build_model_variant

class CAGSRec(nn.Module):
    """
    Main model class for CAGSRec: Cross-Attention Graph for Sequential Recommendation
    """
    def __init__(self, config, item_num, node_num, relation_num, gcn=None):
        ...
        self.model_variant = self.args.model_variant
        attn_dimension = (self.args.conv_layer_num + self.args.short_conv_layer_num) * self.args.dim
        
        self.long_term_gnn, self.short_term_gnn, self.temporal_seq_attn_layer, self.cross_attn_layer = build_model_variant(self.args, relation_num)
