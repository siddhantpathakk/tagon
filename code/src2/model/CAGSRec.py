...
from .FFN import PointWiseFeedForward, SimpleFeedForward
from .improvedGNN import LongTermGNN, ShortTermGNN
from .attention_layers import TemporalSequentialAttentionLayer, CrossAttentionLayer
...
class CAGSRec(nn.Module):
    def __init__(self, config, item_num, node_num, relation_num):
        ...
        attn_dimension = (self.args.conv_layer_num +
                              self.args.short_term_conv_layer_num) * self.args.dim
        ...
        self.temporal_seq_attn_layer = TemporalSequentialAttentionLayer(embed_dim=attn_dimension,
                                                                        num_heads=self.arg.TSAL_head_num,
                                                                        dropout=self.arg.attn_drop,
                                                                        feedforward=self.FFN).to(self.device)
        
        self.cross_attn_layer = CrossAttentionLayer(embed_dim=attn_dimension,
                                                    num_heads=self.arg.CAL_head_num,
                                                    dropout=self.arg.attn_drop,
                                                    feedforward=self.FFN).to(self.device)
        
        self.long_term_gnn = LongTermGNN(dim = self.arg.dim,
                                         conv_layer_num=self.arg.conv_layer_num,
                                         relation_num=relation_num).to(self.device)
        
        self.short_term_gnn = ShortTermGNN(dim=self.arg.dim,
                                           conv_layer_num=self.arg.short_conv_layer_num,
                                           relation_num=relation_num).to(self.device)
        
        self.predict_emb_w = nn.Embedding(item_num, (self.args.conv_layer_num + self.args.short_term_conv_layer_num) * self.args.dim, padding_idx=0).to(self.device)
    ...
    def forward(self, X_user_item, X_graph_base):
        ...
        item_embeddings = self.cross_attn_layer(user_embeddings, item_embeddings)
